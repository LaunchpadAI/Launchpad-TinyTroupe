...
const MODIFICATION_QUESTIONS = [
  ...
  "Whould you buy this property? If not, what would make you more likely to buy?",
...
const TEST_PROPERTIES = [
  ...
  {
    name: "Hillsborough Historic Estate",
    data: {
      ...
      sqft: "6,818",
      yearBuilt: "1917",
      style: "Historic Carriage House",
      features: "Historic charm, Mature trees, Solarium balcony access, Fully equipped media room, Modern living, Complete seclusion and privacy, Upstairs sunroom",
      neighborhood: "Lower Hillsborough",
      description: "Step into a piece of history with this extraordinary Hillsborough property, originally the 5-stall carriage house for the infamous 16-acre Fagan Estate. Transformed in 1971, the carriage house is now a luxurious single-family residence, designed to cater to modern living, while beautifully preserving the historic charm. Spanning approximately 7,000 square feet, the home includes 7 bedrooms, 4.5 baths, fully equipped media room, home gym & upstairs sunroom with solarium balcony access. The multi-level layout wraps around meticulously designed gardens, offering picturesque views throughout. The exterior features over half an acre of vast garden space. Enjoy the ivy-covered trellis, elevated terrace with heated saltwater pool & spa & rolling grass-covered yards. Garden pathways lead to a charming children's play area. Mature trees surround the property, ensuring complete seclusion and privacy. Located in the prestigious Lower Hillsborough area, this property sits on a private, quiet street with no through traffic. Excellent Hillsborough North schools are a significant draw for families, while the proximity to downtown Burlingame & San Mateo ensures you're never far from amenities. Quick connections to international airports, San Francisco, & Silicon Valley add to the convenience."
    }
  }
];
...
export default function PropertyFocusGroup() {
  ...
  const handleImageUpload = async (files: FileList) => {
    ...
    try {
      ...
      const newImages = result.images.map((img: any) => img.base64);
...
${hasImages ? `\nProperty Images: Please analyze the ${property.images?.length || 0} property image${(property.images?.length || 0) > 1 ? 's' : ''} provided to better understand the property's condition, styling, and features.\n` : ''}

As high net worth individuals with experience in luxury real estate, please discuss:
${customQuestions.map((q, i) => `${i + 1}. ${q}`).join('\n')}

Please provide specific, actionable recommendations based on your expertise${hasImages ? ', market knowledge, and visual analysis of the property images' : ' and market knowledge'}.
      `;
      ...
      <div className="bg-white rounded-lg shadow-lg p-6 mb-6">
        ...
        <div className="mt-6">
          ...
          <p className="text-sm text-gray-600 mb-3">
            Upload images of the property to enhance the focus group discussion. Agents will analyze these images alongside the property details.
          </p>
